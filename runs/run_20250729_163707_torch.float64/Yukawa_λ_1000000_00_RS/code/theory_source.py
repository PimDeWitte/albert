...
class Yukawa(GravitationalTheory, QuantumMixin):
    ...
    def get_metric(self, r: Tensor, M_param: Tensor, C_param: Tensor, G_param: Tensor, t: Tensor = None, phi: Tensor = None) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        ...
        g_tt = -m
        # <reason>chain: For weak fields, use higher-order Taylor expansion for PPN accuracy</reason>
        # For Yukawa with vanishing correction at large r: m = 1 - rs/r
        # Need exact expansion of 1/(1-x) to capture PPN beta correctly
        x = rs / r * (1 + self.alpha * yukawa_factor)
        # <reason>chain: Use Padé approximant for better accuracy than Taylor series</reason>
        # Padé[2,1] approximation: (1 + 2x/3 + x²/3)/(1 - x/3)
        # This matches 1/(1-x) up to O(x³) and has correct pole structure
        g_rr = torch.where(
            m > 0.5,
            (1 + 2*x/3 + x**2/3) / (1 - x/3),
            1 / m
