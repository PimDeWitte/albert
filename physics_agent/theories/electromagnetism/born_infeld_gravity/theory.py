"""
Born-Infeld Gravity Theory

A theory that extends Born-Infeld nonlinear electrodynamics to gravity.
Just as Born-Infeld electrodynamics removes the infinite self-energy of 
point charges, Born-Infeld gravity attempts to remove singularities from
black holes while preserving key features.

Key principles:
1. Maximum field strength (electric and gravitational)
2. Smooth interpolation between weak and strong field regimes
3. Regular black hole solutions without singularities

<reason>chain: Tests nonlinear modifications of both electromagnetism and gravity</reason>
"""
...
BornInfeldGravity.sweep = {
    'b_em': [1e18, 1e20, 1e22],  # V/m (Born-Infeld electric field scale)
    'lambda_bi': [0.01, 0.1, 1.0],  # Regularization parameter
}

# Preferred parameters
BornInfeldGravity.preferred_params = {
    'b_em': 1e20,  # ~100 times classical electron field
    'lambda_bi': 0.1,  # Moderate regularization
}


# Import missing constant
try:
    from physics_agent.constants import PLANCK_CONSTANT
except ImportError:
