...
class KerrNewmanTheory(GravitationalTheory):
    """
    Kerr-Newman metric for a rotating, charged black hole.
    """

    def __init__(self, M: float = 1.0, a: float = 0.5, q_e: float = 0.3, G: float = 1.0, c: float = 1.0):
        """
    ...
    def get_metric(self, r: Tensor, M_param: Tensor, C_param: float, G_param: float, a: float = None, q_e: float = None) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        """
        Calculates the Kerr-Newman metric in the equatorial plane.
        """
        a = self.a if a is None else a
        q_e = self.q_e if q_e is None else q_e
        ...
        # r_Q = q_e * rs/2, similar to how a_val = a * rs/2
        r_Q = q_e * rs / 2  # Dimensionless q_e scaled by rs/2
        r_Q_squared = r_Q**2
