...
class ScatteringAmplitudeValidator(PredictionValidator):
    ...
    def calculate_theory_prediction(self, theory, process: str, energy: float) -> Tuple[float, float]:
        ...
        # Classical theories cannot predict quantum scattering
        if 'newtonian' in theory_name_lower or 'newton' in theory_name_lower:
            # Newtonian gravity cannot predict particle physics
            return None, None
    ...
    def validate(self, theory, process='ee_to_mumu', **kwargs) -> ValidationResult:
        ...
        # Handle case where theory cannot make predictions
        if theory_value is None:
            result.passed = False
            result.notes = "Theory cannot predict particle physics scattering"
