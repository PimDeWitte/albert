...
class Kerr(GravitationalTheory):
    ...
    def __init__(self, a: float = 0.5):
        ...
        # <reason>chain: Mark as stationary and axially symmetric for solver optimization</reason>
        self.is_stationary = True  # Independent of t
        self.is_axially_symmetric = True  # Independent of φ
    ...
    def get_metric(self, r: Tensor, M_param: Tensor, C_param: float, G_param: float, 
        ...
        if discriminant >= 0:
            ...
            if not torch.is_tensor(discriminant):
                discriminant = torch.tensor(discriminant, dtype=r.dtype, device=r.device)
