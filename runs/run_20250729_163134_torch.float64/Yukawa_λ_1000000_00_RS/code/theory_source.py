...
class Yukawa(GravitationalTheory, QuantumMixin):
    ...
    def get_metric(self, r: Tensor, M_param: Tensor, C_param: Tensor, G_param: Tensor, t: Tensor = None, phi: Tensor = None) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        ...
        g_tt = -m
        # <reason>chain: For weak fields where m ≈ 1, use Taylor expansion to avoid precision loss</reason>
        # 1/m = 1/(1-x) ≈ 1 + x + x² where x = rs/r*(1+α*e^(-r/λ))
        x = rs / r * (1 + self.alpha * yukawa_factor)
        # Use exact formula when close to horizon, Taylor expansion for weak field
        g_rr = torch.where(m > 0.5, 1 + x + x**2, 1 / m)
        g_pp = r**2
